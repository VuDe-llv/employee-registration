  ...
  return (
    <>
              ...
              Object.entries(groupedData).map(([ngayLaoDong, sessions], groupIndex) =>
                Object.entries(sessions).map(([buoiLaoDong, rows], sessionIndex) =>
                  rows.map((row, rowIndex) => {
                    ...
                    const isRegisteredInDayAndSession = hasRegisteredInSession(ngayLaoDong, buoiLaoDong);
                      ...
                      <tr key={`${groupIndex}-${sessionIndex}-${rowIndex}`}>
                        ...
                        <td>
                          <button
                            ...
                            onClick={() => handleViewDetail(row)} // Gọi hàm hiển thị modal chi tiết
                          >
                          ...
                          {row.trangThai === "Đã đăng ký" && row.maNguoiDung !== user.maNguoiDung && (
                            <button
                              type="button"
                              className="btn btn-outline-secondary btn-sm ms-2"
                              disabled
      ...
      {isDeleteModalVisible && (
        ...
        >
          <p>Bạn có chắc chắn muốn hủy đăng ký lao động không? Thông tin đăng ký của bạn sẽ bị xóa.</p>

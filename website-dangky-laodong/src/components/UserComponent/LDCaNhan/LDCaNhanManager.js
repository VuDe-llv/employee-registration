...
const LDCaNhanManager = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const formattedData = laoDongCaNhanResponse.data.map((item) => ({
        ...
        ngayBatDau: tuanldMap[item.maTuanLaoDong] || "",
        ngayKetThuc: tuanldMap[item.maTuanLaoDong] || "",
  ...
  const handleConfirmDelete = (deleteData) => {
    axios
      .put(`https://localhost:7086/api/LaoDongCaNhan/unsub/${deleteData.maLDCaNhan}`, {
  ...
  const handleSave = (data) => {
    if (editingData) {
      axios
      .put(`https://localhost:7086/api/LaoDongCaNhan/${editingData.maLDCaNhan}`, {
  ...
  return (
    <div className="row mb-3">
      ...
      <LDCaNhanList data={paginatedData} onEdit={handleEdit} onDelete={handleConfirmDelete} khuvucs={khuvucs} />

      {modalVisible && (
        <LDCaNhanModal title="Thông tin đăng ký" isVisible={modalVisible} onClose={handleCancel}>
          <LDCaNhanForm selectedItem={editingData} onSave={handleSave} onCancel={handleCancel} khuvucs={khuvucs} />
        </LDCaNhanModal>
      )}
    </div>
  );
